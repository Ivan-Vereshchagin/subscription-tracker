...
# Сбер, строка 1: "05.06.2024  12:30  Супермаркеты  -1 250,00  45 000,00"
# группы: (дата)(время)(категория)(сумма)(остаток)
TRANSACTION_RE = re.compile(
    r'^(\d{2}\.\d{2}\.\d{4})\s+(\d{2}:\d{2})\s+(.+?)\s+([+]?[\d ]+,\d{2})\s+([\d ]+,\d{2})\s*$'
)

# Сбер, строка 2: "05.06.2024  123456  ООО Пятёрочка. Операция по карте ****1234"
# группы: (дата)(код авторизации)(название магазина...)
...
# Т-банк, главная строка: "13.05.2026  13.05.2026  -650.00 ₽  +6.50 ₽  Оплата в ООО Пятёрочка  9033"
...
# Т-банк, строка-продолжение: "17:46  18:01  Y.M*VKUSNOITOCHKA" или "17:46  18:01  MOSKVA RUS"
TBANK_CONT_RE = re.compile(r'^\d{2}:\d{2}\s+\d{2}:\d{2}(?:\s+(.+))?$')

TBANK_CITY_RE = re.compile(r'^[A-Z0-9][A-Z0-9 \-]+$')

# Описания, которые точно не являются подписками — переводы, кэшбэк, пополнения
